...
#### Phase 11 implementation record - 2026-09-16

Measured on Windows 11 against local `vite preview`, driven by Playwright 1.62.1. Engines: Chrome 152.0.7977.84 and Edge 153.0.4234.32 (installed channels), Playwright Firefox 153.0 (build v1538) and WebKit 26.5 (build v2336), the latter two downloaded for this phase because only a stale Chromium build was cached. Lab results on one desktop machine, not field data.

- This was QA, not development, and it ended with **no product-code change**. `src/`, `api/`, `shared/`, `index.html`, and `public/` are byte-identical to `348c35e`. The only edits are test infrastructure: three new Playwright projects, two new specs, one evidence script, and one `package.json` script.
- `playwright.config.ts` gains `firefox`, `webkit`, and `edge` projects whose `testMatch` selects only `phase11-cross-browser.spec.ts` and `phase11-evidence.spec.ts`. The accepted Phase 0-10 specs stay Chrome-only deliberately: several assert Chromium-specific diagnostics - `emulateMedia({ forcedColors })`, renderer draw-call budgets, CDP throttling - that a second engine would fail for tooling reasons rather than product defects. Running the whole suite in four engines would have produced a flood of non-defects and pressure to weaken accepted gates. `workers: 4` is unchanged.
- `tests/e2e/phase11-cross-browser.spec.ts` runs the same gates in each of the four engines: the full Hero -> Planify -> Stack -> Selected Work -> Journey -> Credentials -> AI -> Contact -> Footer journey, forward and reverse transition, a ten-width device matrix (320/360/375/390/430 phones, 768 tablet portrait, 1024x768 tablet landscape, 740x360 short landscape, 1280x720, 1440x900, the first eight under touch and the last two under a fine pointer), touch menu open/Escape/focus return, 44 px target floor, launcher collision, keyboard traversal and focus indicators, certificate-viewer focus containment and return, shared inline/floating AI conversation, slow network, provider error plus retry, connection failure, reduced motion, WebGL-unavailable, and context-loss. It is written to engine-portable APIs only - no `isMobile` (Firefox rejects it), no `forcedColors`, no CDP - and slow-network behavior is driven by `page.route` delays, matching the Phase 8 mock convention.
- **Cross-engine scene parity is exact.** At 1440x900 all four engines report `tier=full`, `mode=enhanced`, `reason=capability-full`, exactly one canvas, 33 draw calls, 13,958 triangles, DPR 1. The one-canvas invariant, the reduced/static tier selection, and both fallback paths behave identically in Gecko, WebKit, and both Chromium channels.
- **Five candidate defects were found and every one resolved to a test bug, not a product defect.** Each was diagnosed empirically before any edit. (1) Launcher collision failed in Firefox and WebKit: `scroll-behavior: smooth` is global in `src/index.css`, so the geometry was read mid-scroll, and Gecko/WebKit animate longer than Chromium; measuring at the settled offset shows all four engines place the launcher identically at 778-828 px with the nearest link ending at 715 px. The spec now waits for the scroll offset to stabilise. (2) Certificate focus containment failed because the viewer is a native `<dialog>`, whose dialog role is implicit, so a `[role="dialog"]` attribute selector never matched it. (3) Context-loss failed in Edge because the spec fired `loseContext()` before the slot reported `enhanced`, racing the R3F `ContextLossMonitor` listener registration - the accepted `phase3.spec.ts` already gates on `enhanced` first, and the new spec now does the same. (4) The reverse-transition gate failed in WebKit under load for the same smooth-scroll reason as (1): a programmatic jump to the top issued while the forward smooth scroll was still in flight is overtaken by that animation and lands back down the document. (5) The keyboard gate failed in WebKit for the platform reason below.
- **Two WebKit platform traits, classified as environment limitations rather than defects.** WebKit ships Safari's default full-keyboard-access behavior, where Tab visits buttons and form controls but never `<a href>`: probing six Tab presses returns links in Chrome and Firefox (`Skip to content` first) and only buttons in WebKit. The skip-link ordering gate is therefore asserted where the engine can express it and skipped, with the reason recorded, in WebKit; a separate engine-independent gate confirms activating the skip link moves focus to `#main-content` everywhere. Separately, in the modal certificate viewer WebKit's Tab cycle parks on the inert `<body>` between passes (BODY -> DIALOG -> BUTTON -> BODY) while Chromium keeps every stop inside the dialog subtree. `#root` carries `inert` and the dialog is `showModal()`, so no interactive element outside the dialog is ever reachable and Escape still closes and restores focus. The gate asserts that contract - no reachable interactive content outside the dialog - rather than a Chromium-specific cycle.
- **AI failure paths were validated without any provider traffic, and the guarantee is structural rather than a promise.** `vite preview` serves static `dist/` output and never executes the Vercel functions in `api/`, so `POST /api/chat` returns 404; `dist/` contains no chat endpoint and the built bundle contains no `openrouter` string. No automated request could reach OpenRouter even unmocked. On top of that, every AI gate mocks `**/api/chat`. Verified in all four engines: mocked success, pending/`thinking` status, 500 mapped to the retryable "temporarily unavailable" alert with a working Retry that succeeds on the second attempt, aborted connection mapped to the connection error, and a 1.2 s delayed response that holds the pending state and resolves. With the assistant failing, Contact, Credentials, and the footer stay usable with no horizontal overflow.
- **Both 3D fallback paths converge on the same static hero, in every engine.** WebGL-unavailable yields `tier=static`, `reason=webgl-unavailable`, zero canvases, and a visible `.hero-fallback`; context-loss yields `reason=context-lost` and the same result. The two evidence stills are byte-identical in size because the fallback composition is the same regardless of cause - that convergence is the finding, not two separate ones.
- **Not validated, and not claimed:** no real iOS Safari, macOS Safari, or Android Chrome device was available. Playwright WebKit 26.5 on Windows is useful cross-engine coverage but is not Safari - it reports `gpu=Apple GPU` under a Windows host, which no real device would. The Section 27 row prioritising real iOS Safari and Android Chrome smoke tests is therefore outstanding and carries into Phase 12. Everything here used emulated viewports in headless engines; no physical device, no real browser zoom UI, and no orientation-change hardware event were exercised.
- `tests/e2e/phase11-evidence.spec.ts` is gated behind `PHASE11_EVIDENCE=1` and run by `npm run evidence:phase11` on one worker, matching the Phase 3B/4/10 convention. It is deliberately lean: one desktop still per engine plus the device and fallback conditions captured once in the Chrome project, eleven stills totalling 3.8 MB in `artifacts/phase11/`, no video.
- **The worker pool needed the Phase 10 treatment again, and got the Phase 10 remedy rather than a lower worker count.** Adding four engine projects took the suite from 127 to 270 collected tests, and single tests began failing intermittently in a different file each run while passing in isolation - `phase2.spec.ts` once, `phase9-visual.spec.ts` once - which is the same signature Phase 10 recorded at eight workers. The cause was context churn, not the worker count: a context per width per engine meant forty browser contexts per run on top of the existing suite. The width matrix now runs inside one context per input mode, reloading the document at each width, so coverage is identical while the contexts drop from forty to eight. `workers: 4` is unchanged, and the accepted specs were not touched.
- Regression at the end of the phase: lint, typecheck, 94 unit tests in 17 files, `git diff --check`, the production build, and the dev-lifecycle StrictMode check (five refresh cycles, exactly one canvas each, pointer/idle/recession/simulated-context-loss all correct) all pass. The browser suite is 202 passed / 36 skipped in 2.0 min across four projects, clean on four consecutive full runs.
- Preserved unchanged: one R3F canvas, the StrictMode lifecycle patch, native scroll, the Hero-to-Planify story, Selected Work, the Engineering System Map, Journey/Credentials, the unified Phase 8 assistant session, Phase 9 contact/footer/SEO behavior, Phase 10 accessibility/performance hardening, `openrouter/free`, and every progressive-enhancement fallback. No accepted test was weakened; the four corrections were to new Phase 11 gates that asserted the wrong thing.

#### Phase 11 corrective record - real iPhone Safari Hero continuity - 2026-09-16

A real iPhone Safari smoke test found the Hero 3D vanishing part-way through the scroll toward Planify, leaving a dark empty band before Planify entered, and returning on reverse scroll. The read-only audit proved it was neither WebGL context loss nor a Safari lifecycle bug.

- **Root cause: an incomplete mobile override, not an engine defect.** The two `--planify-source-release` rules released the Hero source to opacity 0 whenever `data-transition-enhanced="true"`, in every mode. Everything that receives that release is cinematic-only - `.planify-handoff-frame` stays `visibility: hidden` outside `cinematic`, and every Planify reveal rule is gated on it - so `simplified` released the visual into nothing. The slot then sat empty on screen until Planify scrolled in. The author had already recognised this asymmetry for the sibling elements: `@media (max-width: 56rem)` softens the Hero copy and environment from x0.94 to x0.36. The equivalent floor for the source release was simply never added.
- **Proof it was not the scene lifecycle.** Through the whole blank band the slot reported `canvases: 1`, `data-scene-mode="enhanced"`, `data-scene-reason="capability-reduced"`, a live `376x360` canvas, and zero context-related console messages. `data-scene-active` only turned false at progress 0.954, well after the band opened at 0.607, so the offscreen pause and the IntersectionObserver are not implicated. Measured blank band at 390x844: scrollY 420 to 780, 420 px of scrolling with a ~347 px empty slot on screen.
- **Not Safari-specific.** Playwright WebKit and Chrome at 390x844 produced byte-identical numbers, and the static tier reproduced it too - with WebGL forced off the same rule fades `.hero-fallback` to 0 over the same band. Desktop cinematic at 1440x900 had zero blank samples, because scene 0.4927 and handoff 0.5073 cross-fade to exactly 1. Windows Playwright WebKit did not miss this for engine reasons; it was a coverage gap. Every Phase 11 gate was structural - overflow, section visibility, canvas count - and every evidence still was captured at a scroll rest position, never inside the 0.62 to 1.0 band.
- **Fix: one CSS rule, mode-scoped, in `src/styles/planify.css`.** A `[data-transition-mode='simplified']` variant floors both the ready `.hero-scene-layer` and the static `.hero-fallback` at `calc(1 - var(--planify-source-release) * 0.36)`, mirroring the factor the Hero copy already uses on narrow viewports, so the visual hands over by dimming to 64% instead of vanishing. Gated on the mode rather than a width breakpoint because the mode is exactly the condition under which no handoff frame exists. Cinematic is untouched and still releases fully into the flying frame. No JavaScript, lifecycle, offscreen-pause, R3F, StrictMode, or transition-timing change; the accepted Phase 3B/4 behavior and the one-canvas invariant are unchanged.
- **Regression gates, proven against the unfixed build.** `tests/e2e/phase11-cross-browser.spec.ts` gains three gates running in all four engines: mid-transition continuity at 320/375/390/430 under touch, the same walk with WebGL unavailable, and a desktop gate asserting the cinematic crossfade still sums to 1. The continuity gates assert the user outcome - while the slot is on screen and Planify has not arrived, the active visual stays above 0.25 - rather than pinning the 0.64 the fix produces. Both were verified to fail on a stashed, rebuilt copy without the fix (`scene-layer opacity=0` and `fallback opacity=0` across scrollY 424 to 848) and to pass with it, 12/12 across Chrome, Edge, Firefox, and WebKit.
- Validation: lint, typecheck, 94 unit tests, `git diff --check`, build, and the dev-lifecycle StrictMode check all pass. Full browser suite 214 passed / 40 skipped in 2.5 min across four projects, clean on the first run. Bundle impact is CSS only: 123.01 -> 123.34 kB raw, 23.72 -> 23.75 kB gzip. Every JavaScript chunk is byte-identical. Evidence: `artifacts/phase11/mobile-390-mid-transition.png` shows the formerly blank region carrying the dimmed 3D scene as Planify enters.
- Optional/future, deliberately not bundled: a 1200x800 coarse-pointer screen also resolves to `simplified` while `@media (max-width: 56rem)` does not match, so the Hero copy there still fades to 0.06 with no handoff frame. It produces no blank band, because Planify is already in view at that width, so it is recorded rather than fixed.
- Outstanding acceptance gate: real iPhone Safari re-test of the same scroll-down-then-up pass. Playwright reproduced the defect and now gates the fix, but only the device confirms it under real dynamic-toolbar scrolling. Franci will run it manually.
...
#### Phase 12 implementation record - 2026-09-16

A read-only production readiness audit of `2c69bdf` against the live Vercel deployment. **No release blocker was found and no product code was changed.** The only edits are documentation: this record and the release/rollback section the acceptance criteria require.

- **Production is live and byte-identical to HEAD.** `https://portfolio-azure-nu-94.vercel.app` returns 200, and the served `index-BY-BC4N-.css` is byte-for-byte identical to a local `npm run build` (123,341 bytes), including the Phase 11 simplified-mode continuity rule. The served JS chunk hash matches too, so production carries the accepted Phase 0-11 state with nothing stale. Deployment is Vercel's Git integration from `main`; the project link lives in the untracked `.vercel/project.json`.
- **Release chain verified without consuming provider quota.** `OPTIONS /api/chat` returns 204 and `GET /api/chat` returns 405 with a generic `Method not allowed`; both are handled before any upstream call. `/Franci-Hoxha-CV.pdf` (102,301 B), `/og-image.png`, `/planify-preview.png`, `/favicon-fh.svg`, and a certificate JPEG all return 200 with correct content types and sizes matching the repository. No POST was issued, so no OpenRouter request was made.
- **`/api/chat` security contract holds.** `openrouter/free` is unchanged. The key is read only through `process.env.OPENROUTER_API_KEY` and a missing key degrades to a generic 503 rather than an error trace. The system prompt is server-owned and built from an allowlisted subset of canonical data. `validateChatRequest` enforces a `user`/`assistant` role allowlist, a 16 kB body bound, 12 messages, 1,500 characters per message, 8,000 total, control-character rejection, and a final-message-must-be-user rule. The upstream call is abort-controlled at 12 s. Every client-visible failure is a generic `AI service unavailable` with a stable code; provider status text is written to server logs only. Bundle scans found no `sk-or-v1`, no `OPENROUTER_API_KEY`, and no `Bearer` string, and no source maps are emitted.
- **Rate limiting: a recorded production limitation, not a faked one.** There is no rate limiter. A per-instance in-memory counter was deliberately not added: Vercel functions scale horizontally and recycle, so it would throttle inconsistently while implying a guarantee that does not exist, and durable distributed limiting needs paid infrastructure this project does not have. The exposure is bounded instead by the request validator, `max_tokens: 300`, the 12 s timeout, and a free-tier model. Accepted limitation.
- **CORS.** The allowlist is `francihoxha.github.io`, `portfolio-azure-nu-94.vercel.app`, and two localhost dev ports. Two observations, both optional rather than blocking: `https://francihoxha.github.io` currently returns 404, so it is a dead entry, and the localhost origins remain allowed in production. Neither is a security boundary for a public unauthenticated endpoint - CORS restricts browsers, not clients - so removing them is hygiene, not hardening, and was left alone.
- **Privacy: no leak. The one candidate was checked and disproved.** The client bundle contains the phone number from the `public: false` / `status: 'draft'` contact record, because bundlers cannot tree-shake object literals out of the shared canonical module. That looked like an exposure until the CV was read: `public/Franci-Hoxha-CV.pdf`, publicly downloadable at 200 from the site, prints `francihoxha@yahoo.com | +355 69 253 8842 | Tirana, Albania` on its contact line. The number is therefore already published deliberately, in a more prominent place than a minified bundle, and the Phase 9 `public: false` flag correctly governs the *page*, where no `tel:` link renders. Recorded as a consistency note, not a defect; the Section 30.1 question 2 decision is unchanged and unanswered. No address beyond city level, and no certificate verification URL, appears anywhere.
- **No analytics, no tracking, no router.** No analytics or monitoring dependency is installed and no `gtag`, `dataLayer`, `sendBeacon`, or third-party tag exists in the source or the build. The only external hosts in the bundle are the two project links, LinkedIn, and attribution URLs inside vendor code. `npm audit --omit=dev` reports 0 vulnerabilities.
- **Release hygiene is clean.** Tracked files contain no `dist/`, `artifacts/`, `test-results/`, `.vercel/`, or `.env`; `.env.example` holds only a commented placeholder. The build output carries no test, spec, or debug artifact. One cosmetic item: the explanatory HTML comment about the deferred SEO fields ships in the production `index.html`. It states an intentional decision rather than anything sensitive, so it was left in place. `artifacts/` is not listed in `.gitignore`, which is harmless while evidence stays uncommitted but would surface as untracked noise after an evidence run - optional future tidy-up.
- **Canonical domain: decided and deferred, and an accepted release limitation rather than a blocker.** Franci's decision is that the Vercel deployment URL must **not** become the canonical address, and that a personal domain may follow later. Every URL-dependent field therefore stays omitted exactly as Phase 9 left it, verified on the live page: zero `<link rel="canonical">` with an `href`, zero `og:url`, zero `og:image` or `twitter:image`, zero `application/ld+json`, and `twitter:card` still `summary`. `public/og-image.png` remains prepared and unreferenced. No product code was changed for canonical SEO. Completing these fields is a self-contained follow-up whenever a permanent domain exists: populate `metadata.canonicalUrl`, publish the canonical link, `og:url`, the absolute `og:image` with its structured fields, `twitter:card: summary_large_image`, and the URL-dependent JSON-LD. `validatePortfolio` deliberately does not assert `canonicalUrl`, so that change needs no validator work. Recorded as **deferred SEO completion**, not an outstanding defect. The CV meanwhile distributes `portfolio-azure-nu-94.vercel.app` as the contact address, which is a deployment host and was not promoted into a canonical one.
- **Security headers.** The deployment sends Vercel's default `Strict-Transport-Security` (2 years, `includeSubDomains`, `preload`) but no `X-Content-Type-Options`, `Referrer-Policy`, `X-Frame-Options`, or CSP, because no `vercel.json` exists. For a static, cookieless, unauthenticated portfolio that collects nothing, the residual risk is low, and a strict CSP would have to accommodate the accepted GSAP inline styles and the WebGL canvas. Recorded as an optional future hardening step - `nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, and `X-Frame-Options: SAMEORIGIN` are the safe subset - rather than changed inside a release-readiness audit.
- **Documentation corrected.** `README.md` still described the application as reaching Phase 6/7 and pointed four times at `api/chat.js`, a file that has not existed since the TypeScript runtime rewrite; anyone operating a release would have looked for the wrong file. The phase statement, the four `api/chat.ts` references, and the `/api/*.js` convention note are fixed, the validation list now includes `test:e2e` and `test:dev-lifecycle`, and a Deployment, rollback, and recovery section records the single required environment variable, the quota-free release verification steps, and the rollback path.
- **Rollback path.** Vercel keeps every build immutable, so recovery is promoting the previous good deployment from the dashboard or `npx vercel rollback` - instant, with no rebuild and no revert commit needed to restore service; reverting the offending commit afterwards realigns the repository. Assistant failure is isolated by design: a missing or rotated key degrades only the AI surface to its generic unavailable state while the portfolio, CV, and contact paths stay fully usable, so no site redeploy is involved.
- Final regression at `2c69bdf`: lint, typecheck, 94 unit tests in 17 files, `git diff --check`, the production build, and the dev-lifecycle StrictMode check all pass; the browser suite is 214 passed / 40 skipped in 2.5 min across Chrome, Edge, Firefox, and WebKit, clean on the first run.
- **Real iPhone Safari re-test: PASS.** Franci re-ran the Phase 11 scroll-down-then-up pass on a real iPhone. The Hero visual now stays present through the Hero-to-Planify scroll and the blank band is gone on the device that originally exposed it. That closes the acceptance gate the Phase 11 corrective record left open; no further device testing is requested.
- **Real production AI smoke: one authorized call, observed 504.** Franci authorized exactly one real POST to production `/api/chat`. The request `{"messages":[{"role":"user","content":"What kind of software does Franci build?"}]}` returned **HTTP 504** after **12.73 s** with the body `{"code":"SERVICE_UNAVAILABLE","error":"AI service unavailable"}` (`x-vercel-id: fra1::iad1::r87l9-1789575338235-cf20b40241a4`, `x-vercel-cache: MISS`). No second provider call was made.
  - **The failure is upstream latency, not a configuration defect, and the handler's own paths prove it.** A missing `OPENROUTER_API_KEY` returns 503 *before* any fetch, and any non-ok provider status - invalid model slug, 401, 429 - also returns 503. Only the `AbortController` timeout path returns 504, and the elapsed 12.73 s matches `CHAT_LIMITS.upstreamTimeoutMs` of 12,000 ms plus overhead. So the key is present, the upstream request was dispatched, and `openrouter/free` simply did not answer within the budget. This is exactly the free-model instability the Phase 12 risks row anticipated.
  - **Everything the smoke was meant to verify about our own contract passed.** The endpoint is deployed and reachable, CORS and cache headers are correct, the failure is contained, and the client sees only a generic `AI service unavailable` with a stable code - no provider name, status text, model slug, stack, or key material. `openrouter/free` is unchanged. What the smoke did **not** establish is a successful non-empty reply from the provider.
  - Treated as an **accepted limitation of the free tier**, consistent with the rate-limit entry above: the assistant is a best-effort enhancement whose failure is designed to be harmless, and the portfolio, CV, credentials, and contact paths are unaffected. A retry at a quieter moment is the appropriate confirmation, and needs no code change; raising `upstreamTimeoutMs` was considered and rejected here, because Vercel's own function limit bounds it and a longer spinner is a worse visitor experience than a fast, honest fallback.
- **Final release readiness.** No release blocker exists. The deployed build is byte-identical to `2c69bdf`, every automated gate passes, the accepted Phase 0-11 behavior is intact, the real-device gate is PASS, and the two open items are explicitly accepted limitations rather than defects: deferred SEO completion pending a personal domain, and no durable distributed rate limiting. The portfolio is production-ready and already live.
...
All twelve planned phases are implemented and the roadmap is closed. Phase 12 - Production readiness audited the live deployment at `2c69bdf`, found **no release blocker**, and changed no product code; the production verification, the `/api/chat` security assessment, the disproved phone-exposure candidate, the rollback path, the real iPhone Safari PASS, and the single authorized production AI smoke are in the Phase 12 implementation record above.

Three accepted limitations carry forward, none of them defects and none blocking release: **deferred SEO completion** - Franci decided the Vercel URL must not become canonical and a personal domain may follow, so the canonical link, `og:url`, absolute `og:image`, and URL-dependent JSON-LD stay omitted until one exists; **no durable distributed rate limiting** on `/api/chat`, deliberately not faked with a per-instance counter; and **free-model instability**, observed directly when the authorized production smoke timed out upstream at 12 s and returned the designed generic 504 fallback with no provider detail leaked. Each is recorded with the evidence behind it rather than left as an implied to-do.

No further phase is planned. Future work on this repository should start from the accepted state rather than reopening a completed phase, and the optional items already measured and declined - security headers via a `vercel.json`, the dead `francihoxha.github.io` CORS entry, `artifacts/` in `.gitignore`, and the Phase 10 deferred performance items - are recorded in their own phase records with the reasoning that deferred them.
