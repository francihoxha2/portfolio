import type {
  CapabilityItem,
  CapabilityProminence,
  PortfolioData,
} from './portfolio.types.ts'
import { validatePortfolio } from './validatePortfolio.ts'

function capabilityItems(
  items: Array<
    [id: string, label: string, prominence: CapabilityProminence, featured: boolean]
  >,
): CapabilityItem[] {
  return items.map(([id, label, prominence, featured]) => ({
    id,
    label,
    prominence,
    featured,
    status: 'published',
    verificationStatus: 'confirmed',
  }))
}

const portfolioData = {
  identity: {
    name: 'Franci Hoxha',
    title: 'Full-Stack Software Developer',
    heroStatement: 'Building modern software experiences, from idea to production.',
    capabilityLine: 'Web • Mobile • Backend • AI',
    summary:
      'I work across frontend, backend, data, mobile applications, and AI integration to move software from idea toward production.',
  },
  metadata: {
    title: 'Franci Hoxha - Full-Stack Software Developer',
    description:
      'Franci Hoxha is a Full-Stack Software Developer building modern web, mobile, backend, and AI-integrated software experiences.',
  },
  navigation: [
    { id: 'work', label: 'Work', href: '#work', status: 'published' },
    { id: 'stack', label: 'Stack', href: '#stack', status: 'published' },
    { id: 'journey', label: 'Journey', href: '#journey', status: 'published' },
    {
      id: 'credentials',
      label: 'Credentials',
      href: '#credentials',
      status: 'published',
    },
    { id: 'ai', label: 'Ask AI', href: '#ai', status: 'published' },
    { id: 'contact', label: 'Contact', href: '#contact', status: 'published' },
  ],
  contact: [
    {
      id: 'email',
      label: 'Email',
      value: 'francihoxha@yahoo.com',
      href: 'mailto:francihoxha@yahoo.com',
      public: true,
      assistantVisible: false,
      status: 'published',
      verificationStatus: 'repository-current',
    },
    {
      id: 'phone',
      label: 'Phone',
      value: '+355 69 253 8842',
      href: 'tel:+355692538842',
      public: true,
      assistantVisible: false,
      status: 'published',
      verificationStatus: 'repository-current',
    },
    {
      id: 'location',
      label: 'Location',
      value: 'Tirana, Albania',
      public: true,
      assistantVisible: false,
      status: 'published',
      verificationStatus: 'repository-current',
    },
    {
      id: 'linkedin',
      label: 'LinkedIn',
      value: 'View profile',
      href: 'https://www.linkedin.com/in/franci-hoxha-78a174329/',
      public: true,
      assistantVisible: false,
      status: 'published',
      verificationStatus: 'repository-current',
    },
  ],
  projects: [
    {
      id: 'planify',
      title: 'Planify.al',
      category: 'Software Project',
      label: 'Flagship Software Project',
      description: 'Planify is Franci’s flagship software project.',
      featured: true,
      previewAssetId: 'planify-preview',
      previewPosition: '48% 8%',
      link: {
        label: 'View Planify',
        href: 'https://planify.al',
        external: true,
        status: 'published',
        verificationStatus: 'repository-current',
      },
      stack: [],
      highlights: [],
      status: 'published',
      verificationStatus: 'confirmed',
    },
    {
      id: 'barberspot',
      title: 'BarberSpot.al',
      category: 'Software Project',
      description: 'BarberSpot.al is included in Franci’s selected software work.',
      featured: false,
      link: {
        label: 'View BarberSpot',
        href: 'https://barberspot.al',
        external: true,
        status: 'published',
        verificationStatus: 'repository-current',
      },
      stack: [],
      highlights: [],
      status: 'published',
      verificationStatus: 'confirmed',
    },
    {
      id: 'charging-station',
      title: 'Online Charging Station Management System',
      category: 'Software Project',
      description: 'A selected software project focused on charging-station management.',
      featured: false,
      link: {
        label: 'Discuss the project',
        href: '#contact',
        external: false,
        status: 'published',
        verificationStatus: 'confirmed',
      },
      stack: [],
      highlights: [],
      status: 'published',
      verificationStatus: 'confirmed',
    },
  ],
  capabilityGroups: [
    {
      id: 'frontend',
      title: 'Frontend',
      status: 'published',
      items: capabilityItems([
        ['javascript', 'JavaScript', 'primary', true],
        ['typescript', 'TypeScript', 'primary', true],
        ['react', 'React', 'primary', true],
        ['nextjs', 'Next.js', 'primary', true],
        ['html', 'HTML', 'supporting', false],
        ['css', 'CSS', 'supporting', false],
        ['responsive', 'Responsive interfaces', 'primary', false],
        ['frontend-backend', 'Frontend/backend integration', 'primary', false],
        ['pwa', 'PWA-related application work', 'supporting', false],
      ]),
    },
    {
      id: 'backend',
      title: 'Backend & APIs',
      status: 'published',
      items: capabilityItems([
        ['node-api', 'Node.js / API development', 'primary', true],
        ['rest', 'REST APIs', 'primary', false],
        ['python', 'Python', 'primary', true],
        ['fastapi', 'FastAPI', 'supporting', false],
        ['authentication', 'Authentication', 'primary', false],
        ['authorization', 'Authorization', 'primary', false],
        ['business-logic', 'Application/business logic', 'primary', false],
      ]),
    },
    {
      id: 'data',
      title: 'Data',
      status: 'published',
      items: capabilityItems([
        ['mongodb', 'MongoDB', 'primary', true],
        ['database-apps', 'Database-backed applications', 'primary', false],
        ['mysql', 'MySQL', 'supporting', false],
        ['sql-server', 'SQL Server', 'supporting', false],
      ]),
    },
    {
      id: 'engineering',
      title: 'Engineering & Delivery',
      status: 'published',
      items: capabilityItems([
        ['git', 'Git', 'primary', false],
        ['testing', 'Testing', 'primary', false],
        ['debugging', 'Debugging', 'primary', false],
        ['deployment', 'Deployment', 'primary', false],
        ['hardening', 'Production hardening', 'supporting', false],
        ['integration', 'Integration', 'primary', false],
        ['troubleshooting', 'Troubleshooting', 'primary', false],
      ]),
    },
    {
      id: 'mobile',
      title: 'Mobile',
      status: 'published',
      items: [
        {
          id: 'mobile-development',
          label: 'Mobile application development',
          prominence: 'primary',
          featured: false,
          status: 'published',
          verificationStatus: 'confirmed',
        },
      ],
    },
    {
      id: 'ai',
      title: 'AI',
      status: 'published',
      items: [
        {
          id: 'ai-integration',
          label: 'AI integration',
          prominence: 'primary',
          featured: false,
          evidence: 'Existing AI Portfolio Assistant',
          status: 'published',
          verificationStatus: 'confirmed',
        },
        {
          id: 'ai-assisted-development',
          label: 'AI-assisted development',
          prominence: 'supporting',
          featured: false,
          status: 'published',
          verificationStatus: 'confirmed',
        },
      ],
    },
    {
      id: 'secondary',
      title: 'Additional Engineering Language',
      status: 'published',
      items: [
        {
          id: 'java',
          label: 'Java',
          prominence: 'secondary',
          featured: false,
          status: 'published',
          verificationStatus: 'confirmed',
        },
      ],
    },
  ],
  journey: [
    {
      id: 'business-education',
      kind: 'education',
      title: 'Business Administration studies',
      description: 'Business education forms part of the path into software development.',
      status: 'published',
      verificationStatus: 'confirmed',
    },
    {
      id: 'it-support',
      kind: 'experience',
      title: 'Computer Technician & IT Support',
      description:
        'Supported users and systems across Windows, macOS, and Linux while developing practical troubleshooting, reliability, documentation, and operational skills.',
      status: 'published',
      verificationStatus: 'confirmed',
    },
    {
      id: 'masters',
      kind: 'education',
      period: 'Completed July 2026',
      title: 'Master’s degree',
      status: 'published',
      verificationStatus: 'confirmed',
    },
  ],
  credentials: [
    {
      id: 'software-engineering-build-better-software',
      title: 'Complete Software Engineering Course: Build Better Software',
      provider: 'Udemy',
      instructors: ['Yogesh Dahake'],
      completedOn: '2026-06-23',
      durationHours: 15,
      asset: {
        id: 'credential-software-engineering',
        status: 'planned',
        plannedPath: 'public/certificates/software-engineering-build-better-software.jpg',
      },
      status: 'published',
      verificationStatus: 'confirmed',
    },
    {
      id: 'ai-coder-claude-code-coding-agents',
      title: 'AI Coder: Complete Claude Code & Coding Agents Course',
      provider: 'Udemy',
      instructors: ['Ligency', 'Ed Donner'],
      completedOn: '2026-06-21',
      durationHours: 16.5,
      asset: {
        id: 'credential-ai-coder',
        status: 'planned',
        plannedPath: 'public/certificates/ai-coder-claude-code-coding-agents.jpg',
      },
      status: 'published',
      verificationStatus: 'confirmed',
    },
    {
      id: 'succeed-in-age-of-ai',
      title: 'Succeed in the Age of AI',
      provider: 'Udemy',
      instructors: ['Dr. Angela Yu'],
      completedOn: '2026-05-18',
      durationHours: 8.5,
      asset: {
        id: 'credential-age-of-ai',
        status: 'planned',
        plannedPath: 'public/certificates/succeed-in-age-of-ai.jpg',
      },
      status: 'published',
      verificationStatus: 'confirmed',
    },
  ],
  languages: [],
  availability: null,
  assets: [
    {
      id: 'cv',
      status: 'available',
      src: '/Franci-Hoxha-CV.pdf',
    },
    {
      id: 'planify-preview',
      status: 'available',
      src: '/planify-preview.png',
      alt: 'Planify dashboard preview',
      width: 1200,
      height: 628,
    },
  ],
  assistant: {
    suggestedQuestions: [
      'What has Franci built?',
      'What is his technical stack?',
      'What is his professional background?',
      'What AI experience does he have?',
    ],
  },
} satisfies PortfolioData

export const portfolio = validatePortfolio(portfolioData)
